...
$(document).ready(function() {
    ...
    hookAjax({
        ...
        open: function(arg, xhr) {
            ...
            if (requestTypeName == "GET") {
                ...
                if (accountInfo) {
                    ...
                    var dataObject=parseQueryObject(arg[1]);//提取URL中的签名data参数json字符串
                    //过滤非法参数....待实现开始
                    //过滤非法参数....待实现结束
                    var data=JSON.stringify(dataObject);
                    var timestamp = getNowFormatDate();
                    var signParam={};//签名参数
                    signParam.token = token;
                    signParam.timestamp = timestamp;
                    signParam.action=actionName;
                    if (!isEmpty(data, false)) { //如果data内容为空或者为undefined 则data参数不参与签名
                     	signParam.data=dataObject;
                    }
                    var sign = signTopRequestNew(signParam, secret);
                    __token=token;
        ...
        send: function(arg, xhr) {
            if (requestTypeName == "POST") { //post需要单独取出来数据处理(尚未解决post 地址传值混合模式)
                var accountInfo = getAccountInfo();
                if (accountInfo) {
                    var actionName=___url.replace("//","");
                    ...
                    var token = accountInfo.access_token;
                    var secret = accountInfo.secret;
                    var dataObject=parseQueryString(arg[0]);//提取URL中的参数构造成json字符串
                    //过滤非法参数....待实现开始
                    //过滤非法参数....待实现结束
                    var data=JSON.stringify(dataObject);
                    var timestamp = getNowFormatDate();
        	...
        	xhr.setRequestHeader("token",__token);
            xhr.setRequestHeader("timestamp",__timestamp);
            xhr.setRequestHeader("sign",__sign);            
...
function parseQueryString(url) {
    return JSON.stringify(parseQueryObject(url));
}
//把URL参数转为Json对象
...
function signUrlByGet(url) {
    ...
    if (accountInfo) {
        ...
        var secret = accountInfo.secret;
        //url地址参数json对象
        var params = parseQueryObject(__url);
...
function getSignParamUrlStr(url) {
    ...
    if (accountInfo) {
        ...
        var secret = accountInfo.secret;
        //url地址参数json对象
        var params = parseQueryObject(url);
        if (isEmpty(params.data, false) || params.data == "undefined") { //如果data内容为空或者为undefined 则data参数不参与签名
            delete params.data;
        }
        delete params.pageNo;
