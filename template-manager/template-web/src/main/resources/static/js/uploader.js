    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            uploader.on('uploadSuccess', function (file, data) {
                if(data.status == '2'){
                	mini.get("upload-img_name").setValue(data.imgName);
                	mini.get("upload-img_path").setValue(data.imgPath);
                	mini.get("upload-img_url").setValue(data.imgUrl);
                	parentObj.parent().prev(".upload-img-show").attr("src",data.imgUrl);
                }else if(data.status == '3'){
                	parentObj.parent().prev(".upload-img-show").attr("src",data.imgUrl);
                	parentObj.parent().prev(".upload-img_name").val(data.imgName);
                	parentObj.parent().prev(".upload-img_path").val(data.imgPath);
                	parentObj.parent().prev(".upload-img_url").val(data.imgUrl+"?"+generateMixed(6));
                }else{
                	if (data.status == '0') {
                		var progressObj = parentObj.children(".upload-progress");
